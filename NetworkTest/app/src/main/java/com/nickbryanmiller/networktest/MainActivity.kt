package com.nickbryanmiller.networktest
...
class MainActivity : Activity(), EasyPermissions.PermissionCallbacks {
    ...
    private inner class MakeRequestTask internal constructor(credential: GoogleAccountCredential) : AsyncTask<Void, Void, List<String>>() {
        ...
        init {
            ...
            mService = com.google.api.services.drive.Drive.Builder(
        ...
        val dataFromApi: List<String>
            ...
            get() {
                val fileInfo = ArrayList<String>()
                val result = mService!!.files().list()
                        .setPageSize(10)
        ...
        override fun onPostExecute(output: List<String>?) {
            val myOutput: MutableList<String> = output!!.toMutableList()
            mProgress?.hide()
            if (myOutput == null || myOutput.size == 0) {
                mOutputText!!.text = "No results returned."
            } else {
                myOutput.add(0, "Data retrieved using the Drive API:")
    ...
    companion object {
        ...
        private val BUTTON_TEXT = "Call Drive API"
        private val PREF_ACCOUNT_NAME = "accountName"
        private val SCOPES = arrayOf(DriveScopes.DRIVE_METADATA_READONLY)
    }
}
